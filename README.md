...
A `DATA/` directory is used to store synthetic datasets generated by `StarCLIP.ipynb`. To keep the repository lightweight, this folder is not tracked by Git. If you're running the code from scratch, you must first run `StarCLIP.ipynb` to populate `DATA/` with the necessary files.
